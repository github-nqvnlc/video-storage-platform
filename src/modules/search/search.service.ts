...
export class SearchService implements OnModuleInit {
  ...
  constructor(private readonly configService: ConfigService) {
    this.client = new MeiliSearch({
      ...
      apiKey: this.configService.get<string>('meilisearch.apiKey', 'meili_master_key_123456'),
    });
  }
  ...
  private async initIndex() {
    try {
      ...
      await this.videoIndex.updateSettings({
        ...
        filterableAttributes: ['status', 'visibility', 'tags', 'durationSeconds'],
  ...
  async addOrUpdateVideo(doc: VideoSearchDocument): Promise<void> {
    try {
      ...
      this.logger.error(`Failed to index video ${doc.id} in Meilisearch:`, error.message);
    }
  }
  ...
  async deleteVideo(videoId: string): Promise<void> {
    try {
      ...
      this.logger.error(`Failed to delete video ${videoId} from Meilisearch:`, error.message);
    }
  }
